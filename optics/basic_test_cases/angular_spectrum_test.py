...
from classes import SimulationObject, WaveFunctions, Waveform
...
def test_gaussian_1D():
    ...
    wave = Waveform(energy=1.96, simulation=simulation, z=0, distribution_func=functools.partial(F.gaussian_beam_1D, w0=0.5e-3))
    print(wave)
    ...
    U0 = F.gaussian_beam_1D(x, z=0, wavelength=wave.wavelength, w0=0.5e-3)
    ax[0, 0].plot(x, np.abs(U0)**2, color="Red", alpha=0.7, ls="--", label="Theoretical")
    U = F.gaussian_beam_1D(x, z=z, wavelength=wave.wavelength, w0=0.5e-3)
    I = np.abs(U)**2
    ...
    ax[0, 2].plot(x, rel_err) # type: ignore
...
def test_gaussian_2D():
    ...
    wave = Waveform(energy=1.96, simulation=simulation, z=0, distribution_func=functools.partial(F.gaussian_beam_2D, w0=0.5e-3))
    print(wave)
    im = ax[0, 0].imshow(wave.intensity(), cmap="Greys_r")
    ax[0, 0].set(title="Initial Beam (z=0 m)")
    wave.propagate(z, functools.partial(angular_spectrum_method, dim=2))
    print("Peak intensity:", np.max(wave.intensity()))
    ax[0, 1].imshow(wave.intensity(), cmap="Greys_r")
    ax[0, 1].set(title=f"Simulated Beam (z={z:.2f} m)")
    
    x, y = np.linspace(-Lx/2, Lx/2, N), np.linspace(-Ly/2, Ly/2, N)
    X, Y = np.meshgrid(x, y)
    U = F.gaussian_beam_2D(X, Y, z=z, wavelength=wave.wavelength, w0=0.5e-3)
    I = np.abs(U)**2
    print("Peak intensity:", np.max(I))
    ax[0, 2].imshow(I, cmap="Greys_r")
    ...
    print("Maximum Error:", np.max(err))
    print("Average Relative Error:", np.mean(rel_err))
    ax[0, 3].imshow(rel_err, cmap="Greys_r") # type: ignore
...
def test_constant_1D():
    ...
    simulation = SimulationObject(Lx=Lx, Nx=N, Lz=Lz)
    wave = Waveform(energy=1.96, simulation=simulation, z=0, distribution_func=F.const_wave_1D)
    print(wave)
    ax[0, 0].plot(wave.grid, wave.intensity(), color="Black", alpha=0.7, label="Simulated")
    ax[0, 0].set(title="Initial Beam (z=0 m)")
    wave.propagate(z, functools.partial(angular_spectrum_method, dim=1))
